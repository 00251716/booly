...
function updateall() { updateAllProperties(); }
...
function Inspector(divid, diagram, options) {
  ...
  *
  *
  Configurando boton del modal para que actualize los valores de las propiedades
  *
  *
  */
...
Inspector.prototype.buildPropertyRow = function(propertyName, propertyValue) {
  ...
  *
  *
  Cambiando texto de labels
  *
  *
  */
  var td1 = document.createElement("td");
  td1.textContent = propertyName.charAt(0).toUpperCase() + propertyName.slice(1);
  tr.appendChild(td1);
  ...
  if (decProp && decProp.type === "select") {
    ...
    this.updateSelect(decProp, input, propertyName, propertyValue);input
    input.className = "form-control";
    //COMENTAR ESTA LINEA PARA EVITAR QUE LAS PROPIEDADES SE ACTUALICEN
  ...
  if (diagram.selection.count === 1 || !this.multipleSelection) { // single object update
    ...
    for (var name in inspectedProps) {
      ...
      switch (type) {
        ...
        case 'number': value = isNaN(parseFloat(value)) ? 1 : parseFloat(value); break; // Si el número no es válido, se cambia por 1
